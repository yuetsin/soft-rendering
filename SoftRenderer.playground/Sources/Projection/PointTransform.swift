...
func transformPoint<F: FloatingPoint>(point: Vector3D<F>, camera: Camera<F>, width: F, height: F)
  -> Vector3D<F>
{
  ...
  //    _ = transformed.get(row: 2, column: 0) ?? .zero
  ...
  return Vector3D<F>(screenX, screenY, w)
}
