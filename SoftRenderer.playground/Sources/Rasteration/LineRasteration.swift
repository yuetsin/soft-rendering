...
public func rasterize<F: FloatingPoint>(
  canvasWidth: F, canvasHeight: F, _ lineBeginPos: Vector3D<F>, _ lineEndPos: Vector3D<F>,
  handler: (RenderPoint, LinearInterpolate<Double>) -> Void
) {
  let z0 = lineBeginPos.z as! Double
  let z1 = lineEndPos.z as! Double
  let flatBeginPos = Vector2D<F>(lineBeginPos.x, lineBeginPos.y)
  let flatEndPos = Vector2D<F>(lineEndPos.x, lineEndPos.y)

  rasterize(
    canvasWidth: canvasWidth, canvasHeight: canvasHeight, flatBeginPos, flatEndPos,
...
public func rasterize<F: FloatingPoint>(
  canvasWidth: F, canvasHeight: F, _ lineBeginPos: Vector2D<F>, _ lineEndPos: Vector2D<F>,
