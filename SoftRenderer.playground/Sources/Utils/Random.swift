...
extension Float {
  public static func random(_ lower: Float = 0, _ upper: Float = 100) -> Float {
    return (Float(arc4random()) / 4_294_967_296) * (upper - lower) + lower
  }
}
...
public func randomPoint3d(size: CGSize) -> Vector3d {
  return Vector3d(
    Double.random(0, Double(size.width)), Double.random(0, Double(size.height)),
    Double.random(-2, 2))
}

public func randomCIColor() -> CIColor {
  return CIColor(
    red: CGFloat.random(), green: CGFloat.random(), blue: CGFloat.random(), alpha: CGFloat.random())
}
