...
public let defaultCamera = Camera(
  position: defaultCameraPosition, lookingAtPosition: defaultLookingAtPosition,
  upVector: Vector3d(0, 0, 1), cameraFov: 1, cameraAspect: 1, camZn: 1, camZf: 2)
...
public let defaultDirectionalLight = DirectionalLight(
  shininess: 0.75,
  ambient: .white,
  diffuse: .white,
  specular: .white,
  direction: Vector3d(0, 0, -1))
public let defaultPointLight = PointLight(
  ambient: .white,
  diffuse: .white,
  specular: .white,
  attenuation: Vector3d(1, 1, 1),
  position: Vector3d(0, 0, 2))
public let defaultSpotLight = SpotLight(
  cutOff: 0.5, outCutOff: 2.0,
...
public func generateCube(basePoint: Vector3d, size: Double) -> [TextureFragment3D] {
  ...
  fragments.append(
    TextureFragment3D(
      pointA: basePoint + Vector3d(size, size, 0),
      pointB: basePoint + Vector3d(0, size, 0),
      pointC: basePoint + Vector3d(size, 0, 0),
      texImage: NSImage(imageLiteralResourceName: "grass.png"),
      coordsA: .rightBottom,
...
public func generateAxis(length: Double) -> [Line3D] {
  ...
  lines.append(
    Line3D(beginPoint: Vector3d(-length, 0, 0), endPoint: Vector3d(length, 0, 0), color: .red))
  lines.append(
    Line3D(beginPoint: Vector3d(0, -length, 0), endPoint: Vector3d(0, length, 0), color: .green))
  lines.append(
    Line3D(beginPoint: Vector3d(0, 0, -length), endPoint: Vector3d(0, 0, length), color: .blue))
...
public func generateRefrenceLines(distance: Double, count: Int) -> [Line3D] {
  ...
  for i in -count...count {
    ...
    lines.append(
      Line3D(
        beginPoint: Vector3d(-length, distance * Double(i), 0),
        endPoint: Vector3d(length, distance * Double(i), 0),
        color: .white, endColor: .black))
    lines.append(
      Line3D(
        beginPoint: Vector3d(distance * Double(i), -length, 0),
