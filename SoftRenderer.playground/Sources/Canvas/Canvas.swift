...
public struct SRCanvas {
  ...
  mutating public func moveCamera(offset: CGSize) {
    ...
    let moveXVector =
      worldCamera.up.cross(with: cameraPos).normalize() * (Double(offset.width) / moveFactor)
  ...
  mutating public func drawObject(object: ObjectDrawProtocol3D) {
    object.drawOn(
      target: &imageBuffer, canvasSize: imageSize, depthBuffer: &zBuffer, lights: &worldLights,
      camera: worldCamera)
  }
  ...
  mutating public func clearWorldLights() {
