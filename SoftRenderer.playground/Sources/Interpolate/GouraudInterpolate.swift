...
public func GouraudInterpolateWithCorrection(colorA: CIColor, colorB: CIColor, colorC: CIColor,
                                             interp: FragmentInterpolate<Double>,
                                             zA: Double, zB: Double, zC: Double, refZ: Double) -> CIColor {
    
    let u = CGFloat(interp.u), v = CGFloat(interp.v), w = CGFloat(interp.w)
    
    let cA = colorA / CGFloat(zA), cB = colorB / CGFloat(zB), cC = colorC / CGFloat(zC)
    let z = CGFloat(refZ)
    ...
    return CIColor(red: (red * z).normalize(), green: (green * z).normalize(), blue: (blue * z).normalize(), alpha: (alpha * z).normalize())
}
