...
public class Line3D: ObjectDrawProtocol3D {
  ...
  public func drawOn(
    ...
    let pBegin = transformPoint(
      point: lineBeginPos, camera: camera, width: Double(canvasSize.width),
      height: Double(canvasSize.height))
    let pEnd = transformPoint(
      point: lineEndPos, camera: camera, width: Double(canvasSize.width),
      height: Double(canvasSize.height))

    rasterize(
      canvasWidth: Double(canvasSize.width), canvasHeight: Double(canvasSize.height), pBegin, pEnd,
      handler: { point, interp in
        ...
        if singleColor {
          ...
          pixel = color2Pixel(
            color: LinearColorInterpolate(since: lineBeginColor, till: lineEndColor, interp: interp)
          )
        }
