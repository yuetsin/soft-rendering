...
public class ShadingFragment3D: ObjectDrawProtocol3D {
  ...
  public func drawOn(
    ...
    let tColorA = renderLight(
      lights: lights, material: self.material, camera: camera, position: tPointA, normal: normal)
    let tColorB = renderLight(
      lights: lights, material: self.material, camera: camera, position: tPointB, normal: normal)
    let tColorC = renderLight(
      lights: lights, material: self.material, camera: camera, position: tPointC, normal: normal)
    ...
    rasterize(
      canvasWidth: Double(canvasSize.width), canvasHeight: Double(canvasSize.height), pA: pA,
      pB: pB, pC: pC,
      handler: { point, interp in
          ...
          ?? Double.infinity >= point.z
        {
          ...
          let color = GouraudInterpolateWithCorrection(
            colorA: tColorA,
            colorB: tColorB,
            colorC: tColorC,
            interp: interp,
