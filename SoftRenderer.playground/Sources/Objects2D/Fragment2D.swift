...
public class Fragment2D: ObjectDrawProtocol2D {
  ...
  public func drawOn(target pixels: inout [Pixel], canvasSize: CGSize) {
    ...
    rasterize(
      canvasWidth: Double(canvasSize.width), canvasHeight: Double(canvasSize.height), pA: tPointA,
      pB: tPointB, pC: tPointC
    ) { point, interp in
      ...
      if singleColor {
        ...
        let color = GouraudInterpolate(
          colorA: tColorA, colorB: tColorB, colorC: tColorC, interp: interp)
